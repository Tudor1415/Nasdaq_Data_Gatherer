...
class Flacon(object):
    ...
    def __init__(self, app_or_name, registry=None):
        ...
        self.blueprint = flask.Blueprint(
            "flacon", __name__, template_folder="templates"
        )
    ...
    def redirect_to_status(self):
        """Redirect to the flacon index."""
        return flask.redirect(flask.url_for("flacon.status"))

    def status(self):
        return flask.render_template("flacon/status.html")
    ...
    def threads_bt(self):
        ...
        return flask.render_template("flacon/threads.html", threads=threads)
